...
function Lunar(){
 ...
 this.yueLiHTML=function(By,Bm,curJD){
  ...
  for(i=0;i<this.dn;i++){ //遍历本月各日(公历)
    //生成i日的日历页面
    ob = this.lun[i];
    data_attrs = this.object_to_data_attrs(ob);
    ...
    //c2 += ' onclick="showMessD('+i+', event)"';
    //c2 += ' onmouseout ="showMessD(-1)"'; //dzg
    c2 = '<span' + c2 + '>' +ob.d + '</span>'; //公历的日名称

    if(ob.d0 == curJD) c2='<span'+sty_cur+'>'+c2+'</span>'; //今日标识

    var onclick = ' onclick="showMessD(' + ob.d0 + ')" '; 
    if(typeof window.jdMaps == 'undefined') window.jdMaps = {};
    window.jdMaps[ob.d0] = ob;

    cr += '<td'+onclick+sty_body+'width="14%">'+c2+'<br>'+isM+c+'</td>';
 ...
 this.object_to_data_attrs = function (obj) {
    var str = '';
    for(var name in obj) {
	str += ' ' + name + '="' + obj[name] + '" ';
    }
